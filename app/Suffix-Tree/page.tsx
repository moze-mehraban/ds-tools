...
import { 
  ...
  ZoomIn, ZoomOut, Maximize, Type, X, Scissors
} from 'lucide-react';
...
// --- SUFFIX GENERATOR ---
const getAllSuffixes = (text: string, useTerminator: boolean): string[] => {
    const cleanText = useTerminator && !text.endsWith('$') ? text + '$' : text;
    const suffixes = [];
    for (let i = 0; i < cleanText.length; i++) {
        suffixes.push(cleanText.slice(i));
    }
    return suffixes;
};
...
const insertRadix = (root: TrieNode, word: string): TrieNode => {
    ...
    while (remaining.length > 0) {
        ...
        if (commonLen === child.label.length) {
            ...
            // Preserve ID for animation continuity
...
export default function SuffixTrieVisualizer() {
  ...
  const [useTerminator, setUseTerminator] = useState(true); // New Toggle for '$'
  ...
  useEffect(() => {
      ...
      // Insert every suffix in the list
      words.forEach(w => {
          insertFn(newRoot, w);
      });
      setRoot(newRoot);
  ...
  // --- NEW: Suffix Builder ---
  const handleBuildSuffixTrie = () => {
    if (!wordInput) return;
    const cleanInput = wordInput.trim().toLowerCase();
    
    // Safety check for massive strings causing browser hang
    if (cleanInput.length > 25) {
        showToast('error', 'Text too long for visualization (Max 25)');
        return;
    }

    const suffixes = getAllSuffixes(cleanInput, useTerminator);
    setWords(suffixes);
    showToast('success', `Built Suffix Trie for "${cleanInput}"`);
    setWordInput('');
  };

  const handleInsert = () => {
      if (!wordInput) return;
      // Allow '$' for manual insertion if desired
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans overflow-hidden select-none">
      ...
      <header className="bg-white/90 backdrop-blur-sm border-b border-slate-200 p-4 shadow-sm z-50 relative">
        <div className="max-w-[1400px] mx-auto flex flex-col xl:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 w-full xl:w-auto">
            ...
            <div className="flex items-center gap-3">
                <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                    Suffix {trieMode === 'COMPRESSED' ? 'Tree' : 'Trie'}
                </h1>
          ...
          <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-2 rounded-lg border border-slate-200 w-full xl:w-auto">
            
            {/* Input & Suffix Builder */}
            <input 
                type="text" 
                placeholder="Text" 
                value={wordInput} 
                ...
                className="px-3 py-2 rounded-md border border-slate-300 w-32 outline-none text-black font-mono"
            />
            
            {/* Suffix Build Button */}
            <button 
                onClick={handleBuildSuffixTrie} 
                disabled={isAnimating || !wordInput} 
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-md text-sm font-bold shadow-sm transition flex items-center gap-2"
                title="Clear and build Suffix Trie from input"
            >
                <Scissors size={16}/> Build Suffixes
            </button>

             {/* Terminator Toggle */}
             <button 
                onClick={() => setUseTerminator(!useTerminator)}
                className={`px-3 py-2 rounded-md text-xs font-bold border transition ${useTerminator ? 'bg-slate-700 text-white border-slate-700' : 'bg-white text-slate-400 border-slate-300'}`}
                title="Append $ terminator"
             >
                +$
             </button>
            ...
            {/* Manual Controls */}
            <button onClick={handleInsert} disabled={isAnimating} className="bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 px-3 py-2 rounded-md text-sm font-bold shadow-sm transition"><Plus size={16}/></button>
      ...
      >
        {Object.keys(root.children).length === 0 && (
             <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 pointer-events-none">
                ...
                <p className="font-medium text-lg">Empty Suffix {trieMode === 'COMPRESSED' ? 'Tree' : 'Trie'}</p>
                <p className="text-sm">Enter text and click "Build Suffixes"</p>
             </div>
        )}
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input 
                                ...
                                onChange={(e) => setWordInput(e.target.value)} 
                                disabled={isAnimating} 
                                className="flex-1 px-3 py-2 rounded-md border border-slate-300 text-black outline-none font-mono w-full"
                            />
                            <button onClick={() => handleSearch(false)} disabled={!wordInput || isAnimating} className="px-3 bg-indigo-600 text-white hover:bg-indigo-700 disabled:bg-slate-200 rounded-md text-xs font-bold shadow-sm">Exact</button>
